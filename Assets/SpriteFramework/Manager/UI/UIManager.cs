...
namespace SpriteFramework
{
    ...
    public class UIManager
    {
        ...
        /// </summary>
        /// <param name="CheckReverseChange">是否检查窗体的反切</param>
        public LuaBehaviour OpenUIFormLua(string prefabName, bool CheckReverseChange = true) {
            return OnOpenUIFormLua(prefabName, CheckReverseChange);
        }
        ...
        /// </summary>
        /// <param name="CheckReverseChange">是否检查窗体的反切</param>
        private LuaBehaviour OnOpenUIFormLua(string prefabName, bool CheckReverseChange = true) {
            var entity = GameEntry.DataTable.DTSysUIFormDBModel.GetEntity(prefabName);
            if (entity == null) return null;
            if (!entity.CanMulit && IsOpened(entity.Id)) {
                GameEntry.LogError("不重复打开同一个UI窗体  ID:{0} Path:{0}", entity.Id, entity.AssetFullPath);
                return null;
            ...
            if (formBase != null) {
                _openUIFormList.AddLast(formBase);
                SetSortingOrder(formBase, true);
                return formBase as LuaBehaviour;
            }
            ...
            if (formBase == null) {
                ...
                formBase = obj.AddComponent<LuaBehaviour>();
            }
            formBase.Init(entity);
